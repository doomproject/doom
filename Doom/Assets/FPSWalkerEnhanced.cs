/*
 * Origin source: FPSWalkerEnhanced
 * http://wiki.unity3d.com/index.php/FPSWalkerEnhanced
 * Modified by Richard Fleming
 * License CC BY-SA 3
 * If you use this code in a publicly available project, you must allow users to download this source file.
 * */

using UnityEngine;

[RequireComponent(typeof(CharacterController)), RequireComponent(typeof(MouseLook))]
public class FPSWalkerEnhanced : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        m_mouseLook.LookRotation(m_Transform, m_cameraTransform);
    }
    ...
    private void FixedUpdate()
    {
        ...
        //Now check for teleportation
        if(m_teleport)
        {
            m_teleport = false;
            //for now, just teleport
            transform.SetPositionAndRotation(m_teleportTarget.position, m_teleportTarget.rotation);
            m_Transform = transform;
            m_cameraTransform.rotation = m_teleportTarget.rotation;
    ...
    public void DoTeleport(Transform targetPoint)
    {
        m_teleportTarget = targetPoint;
        m_teleport = true;
    }
